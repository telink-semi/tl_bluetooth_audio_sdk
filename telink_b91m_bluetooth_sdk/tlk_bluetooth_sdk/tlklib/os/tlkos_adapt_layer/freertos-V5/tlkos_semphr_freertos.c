...
{
    TLKOS_ASSERT(semphrHandle != NULL);
...
{
    TLKOS_ASSERT(semphrHandle != NULL);
    *semphrHandle = xSemaphoreCreateCounting(maxCnt, initCnt);
    return TLK_ENONE;
}
...
int tlkos_semphr_take(TlkOsSemphrHandle_t semphrHandle, uint32_t blockTimeMs)
{
    ...
    BaseType_t ret       = xSemaphoreTake((QueueHandle_t)(semphrHandle), delayTick);
    return ret == pdTRUE ? TLK_ENONE : -TLK_ETIMEOUT;
}
...
{
    TLKOS_ASSERT(semphrHandle != NULL);
    BaseType_t ret = xSemaphoreGive((QueueHandle_t)(semphrHandle));
 ...
 * @returns   0 indicates success, other values indicate corresponding error codes.
 */
_attribute_ram_code_sec_ int tlkos_semphr_giveFromISR(TlkOsSemphrHandle_t semphrHandle)
{
    TLKOS_ASSERT(semphrHandle != NULL);
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
    BaseType_t ret                      = xSemaphoreGiveFromISR((QueueHandle_t)(semphrHandle), &xHigherPriorityTaskWoken);
