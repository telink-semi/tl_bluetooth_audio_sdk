...
STATIC_ASSERT_THIS_FILE(IS_4BYTE_ALIGN(sizeof(tlkmem1_unit_t)));
...
_attribute_ram_code_sec_ static tlkmem1_unit_t *tlkmem1_get_unit(tlkmem1_ctrl_t *pCtrl, void *ptr)
{
    ...
    if (addr0 < addr1 || addr0 > addr1 + pCtrl->totalSize || pUnit->used == 0) {
        return NULL;
    }
    return pUnit;
}
...
{
    TLKOS_ASSERT(pBuffer != NULL);
    TLKOS_ASSERT(buffLen >= TLKMEM1_COMM_MIN_SIZE);
    TLKOS_ASSERT(!((uint32_t)pBuffer & (TLKMEM1_COMM_ALIGN_SIZE - 1)));
...
void tlkmem1_clean(void *mem)
{
    ...
    pUnit->size = pCtrl->totalSize - (sizeof(tlkmem1_unit_t));
...
{
    TLKOS_ASSERT(mem != NULL);
    TLKOS_ASSERT(size <= (UINT32_MAX - sizeof(tlkmem1_unit_t)));
    if (size == 0) {
        return NULL;
    }
    ...
    if (pUnit != NULL) {
        if (pUnit->size >= size + (sizeof(tlkmem1_unit_t))) { //have extra size to get a new unit.
            //not tail   extra do :
            tlkmem1_unit_t *pTemp = (tlkmem1_unit_t *)((uint32_t)pUnit + (size + (sizeof(tlkmem1_unit_t))));
            pTemp->used           = 0;
            pTemp->size           = pUnit->size - ((sizeof(tlkmem1_unit_t)) + size);
        ...
        pBuffer = ((uint8_t *)pUnit) + (sizeof(tlkmem1_unit_t));
    }
    tlkmem1_leave_critical();
    return pBuffer;
}
...
_attribute_ram_code_sec_ int tlkmem1_free(void *mem, void *ptr)
{
    ...
    if (pNext != NULL && pNext->used == 0) {
        ...
        pUnit->size += (sizeof(tlkmem1_unit_t)) + pNext->size;
    ...
    if (pPrev != NULL && pPrev->used == 0) {
        ...
        pPrev->size += (sizeof(tlkmem1_unit_t)) + pUnit->size;
        if (pPrev->next != NULL) {
