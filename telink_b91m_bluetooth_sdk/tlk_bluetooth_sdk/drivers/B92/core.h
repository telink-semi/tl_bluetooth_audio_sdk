...
#endif
#if BT_ERROR_CODE
extern volatile unsigned int irq_dis_tick;
extern volatile unsigned int irq_en_tick;
extern volatile unsigned int irq_dis_count;
extern volatile unsigned int irq_dis_pc;
extern volatile unsigned int irq_dis_ra;
#endif
...
#if BT_ERROR_CODE
        if (irq_dis_count == 0) {
            irq_dis_tick = (unsigned int)reg_system_tick;
        }
        irq_dis_count++;
#endif
    }
...
_attribute_ram_code_sec_optimize_o2_ static inline unsigned int core_restore_interrupt(unsigned int en)
{
    if (en) {
        ...
        irq_dis_count--;
        if (irq_dis_count == 0) {
            irq_en_tick = (unsigned int)reg_system_tick;
            irq_dis_pc  = read_csr(NDS_MEPC);
