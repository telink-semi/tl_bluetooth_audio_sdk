...
#define DAC_PATH_TEST_EN 1
#define ADC_PATH_TEST_EN (!DAC_PATH_TEST_EN)
#define DAC_CHANNAL_2_EN 1
#define ADC_CHANNAL_2_EN 1

const Alsa_Device Everest = {
    I2C_NAU8821_ADD,
};
...
////		ctrl1_val |= NAU8821_I2S_DF_RIGHT;
////		break;
...
//}
//
static int nau8821_digital_mute(int mute, int direction)
{
    if (mute) {
        if (direction) {
            return regmap_update_bits(Everest.regmap, NAU8821_R31_MUTE_CTRL, NAU8821_DAC_SOFT_MUTE, NAU8821_DAC_SOFT_MUTE);
        } else {
            return regmap_update_bits(Everest.regmap, NAU8821_R31_MUTE_CTRL, NAU8821_ADC_SOFT_MUTE, NAU8821_ADC_SOFT_MUTE);
        }
    } else {
        if (direction) {
            return regmap_update_bits(Everest.regmap, NAU8821_R31_MUTE_CTRL, NAU8821_DAC_SOFT_MUTE, 0);
        } else {
            return regmap_update_bits(Everest.regmap, NAU8821_R31_MUTE_CTRL, NAU8821_ADC_SOFT_MUTE, 0);
        }
    }
}
...
//#define NAU8821_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S20_3LE | SNDRV_PCM_FMTBIT_S24_3LE | SNDRV_PCM_FMTBIT_S32_LE)
//
...
//static bool nau8821_is_jack_inserted(void)
//{
//	bool active_high, is_high;
//	uint16_t status, jkdet;
//
//	regmap_read(Everest.regmap, NAU8821_R0D_JACK_DET_CTRL, &jkdet);
//	active_high = jkdet & NAU8821_JACK_POLARITY;
//	regmap_read(Everest.regmap, NAU8821_R82_GENERAL_STATUS, &status);
...
//static void nau8821_configure_mclk_as_sysclk(void)
//{
////	regmap_update_bits(Everest.regmap, NAU8821_R03_CLK_DIVIDER,
////		NAU8821_CLK_SRC_MASK, NAU8821_CLK_SRC_MCLK);
////	regmap_update_bits(Everest.regmap, NAU8821_R09_FLL6,
////		NAU8821_DCO_EN, 0);
////	/* Make DSP operate as default setting for power saving. */
////	regmap_update_bits(Everest.regmap, NAU8821_R04_FLL1,
////		NAU8821_ICTRL_LATCH_MASK, 0);
//}

//static int nau8821_configure_sysclk(int clk_id, unsigned int freq)
//{
//
//	switch (clk_id) {
//	case NAU8821_CLK_DIS:
//		/* Clock provided externally and disable internal VCO clock */
//		nau8821_configure_mclk_as_sysclk();
//		break;
...
//		 * 12.288Mhz.
//		 */
//		regmap_update_bits(Everest.regmap, NAU8821_R06_FLL3,
...
{
    regmap_write(Everest.regmap, NAU8821_R00_RESET, 0xffff);
    regmap_write(Everest.regmap, NAU8821_R00_RESET, 0xffff);
}
...
//static void nau8821_init_regs(void)
//{
//
//	/* Enable Bias/Vmid */
//	regmap_update_bits(Everest.regmap, NAU8821_R66_BIAS_ADJ,
//		NAU8821_BIAS_VMID, NAU8821_BIAS_VMID);
//	regmap_update_bits(Everest.regmap, NAU8821_R76_BOOST,
...
//		(nau8821_sys_clk_config.vref_impedance << NAU8821_BIAS_VMID_SEL_SFT) |
//		NAU8821_BIAS_TESTDAC_EN);
//
//	/* Disable short Frame Sync detection logic */
//	regmap_update_bits(Everest.regmap, NAU8821_R1E_LEFT_TIME_SLOT,
...
//
//	/* Class G timer 64ms */
//	regmap_update_bits(Everest.regmap, NAU8821_R4B_CLASSG_CTRL,
//		NAU8821_CLASSG_TIMER_MASK,
//		0x20 << NAU8821_CLASSG_TIMER_SFT);
//	/* Class AB bias current to 2x, DAC Capacitor enable MSB/LSB */
//	regmap_update_bits(Everest.regmap, NAU8821_R6A_ANALOG_CONTROL_2,
//		NAU8821_HP_NON_CLASSG_CURRENT_2xADJ |
//		NAU8821_DAC_CAPACITOR_MSB | NAU8821_DAC_CAPACITOR_LSB,
//		NAU8821_HP_NON_CLASSG_CURRENT_2xADJ |
//		NAU8821_DAC_CAPACITOR_MSB | NAU8821_DAC_CAPACITOR_LSB);
//	/* Disable DACR/L power */
//	regmap_update_bits(Everest.regmap, NAU8821_R80_CHARGE_PUMP,
//		NAU8821_POWER_DOWN_DACR | NAU8821_POWER_DOWN_DACL, 0);
//	/* DAC clock delay 2ns, VREF */
//	regmap_update_bits(Everest.regmap, NAU8821_R73_RDAC,
//		NAU8821_DAC_CLK_DELAY_MASK | NAU8821_DAC_VREF_MASK,
//		(0x2 << NAU8821_DAC_CLK_DELAY_SFT) |
//		(0x3 << NAU8821_DAC_VREF_SFT));
//
//	regmap_update_bits(Everest.regmap, NAU8821_R74_MIC_BIAS,
//		NAU8821_MICBIAS_VOLTAGE_MASK, nau8821_sys_clk_config.micbias_voltage);
//	/* Default oversampling/decimations settings are unusable
//	 * (audible hiss). Set it to something better.
//	 */
//	regmap_update_bits(Everest.regmap, NAU8821_R2B_ADC_RATE,
//		NAU8821_ADC_SYNC_DOWN_MASK, NAU8821_ADC_SYNC_DOWN_64);
//	regmap_update_bits(Everest.regmap, NAU8821_R2C_DAC_CTRL1,
//		NAU8821_DAC_OVERSAMPLE_MASK, NAU8821_DAC_OVERSAMPLE_64);
//}
#if 1
uint16_t table_REG[][2] = {{0x0000, 0x0000},
                           {0x0001, 0x0FD6},
                           ...
                           {0x0082, 0x0064}};
#else
uint16_t table_REG[84][2] = {
    {0x0000, 0xffff},
    //
    //		0x0001,0x0FD6, //demo config
    {0x0001, 0x0c56}, //	DAC EN
//		0x0001,0x0656, //	DAC EN R_DAC disable
...
///FLL
    ...
    /// LR Class_G compare Enable
    {0x004B, 0x2037}, //0x2037,  1/4 Full Scale ///0x2007,		1/16 Full Scale
    ///test
    {0x004C, 0x0000},
    ...
    {0x007E, 0x0101},
    //POWER_UP
    {0x007F, 0x003F}, ///DEMO setting
                      //		0x007F,0x0015,	///R DAC disable
...
static void nau8821_init1_regs(void)
{
    delay_ms(150);
    for (int i = 1; i < 84; i++) {
        regmap_write(Everest.regmap, table_REG[i][0], table_REG[i][1]);
        delay_ms(15);
    }
}
...
int nau8821_i2c_probe(void)
{
    ...
    ret = regmap_read(Everest.regmap, NAU8821_R58_I2C_DEVICE_ID, (uint16_t *)&value, 1);
    ...
    //	regmap_update_bits(Everest.regmap, NAU8821_R74_MIC_BIAS,
    //						NAU8821_MICBIAS_POWERUP,NAU8821_MICBIAS_POWERUP);
    //	delay_ms(25);
    //	regmap_update_bits(Everest.regmap,  NAU8821_R01_ENA_CTRL,
    //			            NAU8821_EN_ADCL, NAU8821_EN_ADCL);
