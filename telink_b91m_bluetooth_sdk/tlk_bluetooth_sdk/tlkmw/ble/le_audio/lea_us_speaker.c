...
struct le_speaker_ad_data_complete_local_name
{
...
struct le_speaker_ad_data_incomplete_list_16bit_service_uuid
{
...
static const struct le_speaker_ad_data_incomplete_list_16bit_service_uuid s_adv_service_uuid_list = {
    .header.length = 15,
...
/**
 * @brief       Add GAP/GATT/BAS services for speaker profile.
 * @param[in]   device_name    - GAP device name string.
 * @return      none.
 */
static void lea_us_speaker_add_base_services(const char *device_name)
{
    // GATT & GAP group.
...
/**
 * @brief       Register LE Audio services (PACS/ASCS/VCS/TMAS) for speaker.
 * @param[in]   param   - speaker configuration with initial volume.
 * @return      none.
 */
static void lea_us_speaker_add_lea_services(const struct lea_us_speaker_param *param)
{
...
/**
 * @brief       Create/start extended advertising containing LEA announcements.
 * @param[in]   param   - advertising interval and name.
 * @return      none.
 */
static void lea_us_speaker_set_advertising_data(const struct lea_us_speaker_param *param)
{
    ...
    struct le_speaker_ad_data_complete_local_name ad_complete_name;
    ...
    const struct ltv_data *speaker_ad_data[] = {
    ...
    int32_t adv_len = ltv_pack(speaker_ad_data, adv_data);
...
struct lea_us_speaker_info
{
    bool                                   is_connected;
    bool                                   close_flag;
    uint16_t                               conn_handle;
    lea_us_speaker_operate_finish_callback callback;
};

static struct lea_us_speaker_info s_lea_us_speaker_info = {
    .is_connected = false,
    .close_flag   = false,
};
...
/**
 * @brief       ACL connected callback: cache handle and stop advertising.
 * @param[in]   conn    - host connection descriptor.
 * @return      none.
 */
static void lea_us_acl_connected_callback(struct ble_host_conn *conn)
{
    if (conn->role == BLE_HOST_ACL_ROLE_PERIPHERAL) {
        s_lea_us_speaker_info.conn_handle  = conn->conn_handle;
        s_lea_us_speaker_info.is_connected = true;
...
/**
 * @brief       ACL disconnected callback: restart adv or finish stop callback.
 * @param[in]   conn    - host connection descriptor.
...
static void lea_us_acl_disconnected_callback(struct ble_host_conn *conn, uint8_t reason)
{
    (void)reason;
    if (conn->role == BLE_HOST_ACL_ROLE_PERIPHERAL) {
        s_lea_us_speaker_info.is_connected = false;
        if (s_lea_us_speaker_info.close_flag) {
            ble_host_gap_extend_adv_stop(s_lea_us_ext_adv_handle);
            lea_us_speaker_operate_finish_callback callback = s_lea_us_speaker_info.callback;
            s_lea_us_speaker_info.callback                  = NULL;
            s_lea_us_speaker_info.close_flag                = false;
            if (callback != NULL) {
                callback();
            }
        } else {
 ...
 * @return      none.
 */
void lea_unicast_server_speaker_initial(const struct lea_us_speaker_param *param)
{
    ...
    lea_us_speaker_add_base_services(param->device_name);
    lea_us_speaker_add_lea_services(param);
    ...
    lea_us_speaker_set_advertising_data(param);
}

/**
 * @brief       Start advertising when speaker is idle.
 * @return      none.
 */
void lea_unicast_server_speaker_start(void)
{
    if (s_lea_us_speaker_info.is_connected == false) {
        ble_host_gap_extend_adv_start(s_lea_us_ext_adv_handle);
    }
}
 ...
 * @param[in]   callback    - called when stop completes.
 * @return      none.
 */
void lea_unicast_server_speaker_stop(lea_us_speaker_operate_finish_callback callback)
{
    if (s_lea_us_speaker_info.is_connected == false) {
        ble_host_gap_extend_adv_stop(s_lea_us_ext_adv_handle);
        if (callback != NULL) {
        ...
        ble_host_gap_disconnect_acl(s_lea_us_speaker_info.conn_handle);
        s_lea_us_speaker_info.close_flag = true;
        s_lea_us_speaker_info.callback   = callback;
    }
}
