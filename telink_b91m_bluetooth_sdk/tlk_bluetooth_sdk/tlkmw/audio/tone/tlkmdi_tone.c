...
#include "tone_sbc.h"
//crazy code.TODO:ZEWEN
#ifdef PROJ_GAMESIR_XIAOJI
#include "vendor/GameSir_Xiaoji/audio_mw/tlkaud_mode_mgr.h" // terrible include, clear later
#endif
...
bool tlkmdi_tone_switch(uint16_t handle, uint8_t status)
{
    ...
    if (status == TLK_STATE_OPENED) {
        ...
        tlkaud_set_audio_mode(AUDIO_TONE);
#else
...
#if TLKADU_MIDBUF_ENABLE
        tlkaud_clear_audio_mode(AUDIO_TONE);
#else
