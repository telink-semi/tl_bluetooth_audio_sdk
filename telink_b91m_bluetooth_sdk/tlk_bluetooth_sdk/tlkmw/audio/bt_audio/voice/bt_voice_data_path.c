...
/*
uint8_t msbc_test_pkt[60] = {
    0x01, 0x08, 0xad, 0x00, 0x00, 0xb4, 0xde, 0x00, 0x00, 0x00,
    0xc5, 0x3e, 0x9c, 0xce, 0xae, 0x58, 0xbc, 0x53, 0xe9, 0xcc,
    0xea, 0xe5, 0x8b, 0xc5, 0x3e, 0x9c, 0xce, 0xae, 0x58, 0xbc,
    0x53, 0xe9, 0xcc, 0xea, 0xe5, 0x8b, 0xc5, 0x3e, 0x9c, 0xce,
    0xae, 0x58, 0xbc, 0x53, 0xe9, 0xcc, 0xea, 0xe5, 0x8b, 0xc5,
    0x3e, 0x9c, 0xce, 0xae, 0x58, 0xbc, 0x53, 0xe9, 0xcc, 0x00
};
*/

volatile unsigned int bt_ll_sco_start_tick;
uint8_t               g_bt_sco_id = 0;

/** allow use silent pkt instead of all zero pkt at esco beginning */
volatile uint8_t g_use_silent_pkt_replace = 0;
volatile uint8_t g_first_esco_assembling  = 0;
 ...
 * @return     None
 */
audio_ram_code void bt_voice_receive_sco_frame(uint8_t id, uint8_t *p_data, uint16_t len)
{
    (void)len;
    uint8_t  i   = 0;
    uint8_t  err = 0;
    ...
    // uint16_t syncHead = p_data[1] + (p_data[2] << 8);
    ...
    */
    ...
    if (HFP_CODEC_ID_MSBC == btif_get_hfp_codec()) {
        ...
        if ((bt_ll_sco_start_tick && !clock_time_exceed(bt_ll_sco_start_tick, 400 * 1000)) || btif_power_off_is_ongoing()) {
            bt_voice_cfg.spk_enc_wptr = id & (SPK_ENC_BUFF_NUM - 1);
            p_des                     = g_spk_enc_buff_ptr + bt_voice_cfg.spk_enc_wptr * SPK_ENC_BUFF_SIZE;
            p_des[0]                  = 0;
        ...
        if (msbc_frame_valid && (msbc_is_frame_head(msbc_enc_start) == FALSE)) {
            ...
            if (g_use_silent_pkt_replace && msbc_is_all_zero_pkt(msbc_enc_start)) {
            ...
            } else if ((abs_ram(id - g_msbc_enc_id) == 1) || (0xff == g_msbc_enc_id && 0 == id) || g_first_esco_assembling) {
                /* P1. save msbc frame. */
            ...
            g_msbc_enc_id           = id;
            g_first_esco_assembling = 0;
        } else {
            valid_msbc_frame_cnt = 0;

            if (msbc_frame_valid && msbc_is_frame_head(msbc_enc_start) && g_use_silent_pkt_replace) {
                g_use_silent_pkt_replace = 0;
                tlkapi_trace(BT_AUDIO_DBG_FLAG, BT_AUDIO_DBG_SIGN, "reset use_silent_pkt_replace %d", g_use_silent_pkt_replace);
            }
        }
    } else {
    }
    ...
    //log_b16_general(1, SL16_bt_sco_enc_wptr, bt_voice_cfg.spk_enc_wptr);
    ...
    if (p_data && p_data[0] == HCI_TYPE_SCO_DATA) {
        ...
        if (sco_frame_len > 60) {
            tlkapi_error(BT_AUDIO_DBG_FLAG, BT_AUDIO_DBG_SIGN, "sco_frame_len, err %d %x %d", *(uint32_t *)p_data, p_data, sco_frame_len);
            sco_frame_len = 60;
        }

        if ((sync_hdl & HCI_SYNC_HDR_PSF_FLAG_MASK) == HCI_SYNC_HDR_PSF_GOOD &&
            ((HFP_CODEC_ID_MSBC == btif_get_hfp_codec() && msbc_is_frame_head(msbc_enc_start)) || (HFP_CODEC_ID_CVSD == btif_get_hfp_codec())) && !err) {
            p_des[0] = 0;
        ...
        tlkapi_error(BT_AUDIO_DBG_FLAG, BT_AUDIO_DBG_SIGN, "tvoice_TYPE_SCO, err %d %x %d", *(uint32_t *)p_data, p_data, sco_frame_len);
    ...
    bt_voice_cfg.tick = clock_time() | 1;
#endif
}
 ...
 * @return     None
 */
_attribute_bt_ram_code_ void bt_voice_mic_enc_get_frame(uint8_t id, uint8_t *p_data, uint16_t len)
{
    (void)id;
    uint8_t *p_mic;
    ...
    /* unmute mic in CEVA BT core after hfp is ready */
    //bt_e_scomutecntl_mute_source_setf(0, 0);

    /* copy MIC encoded frame to ESCO buffer */
    p_mic = g_mic_enc_buff_ptr + ((bt_voice_cfg.spk_enc_wptr - 1) & (MIC_ENC_BUFF_NUM - 1)) * MIC_ENC_BUFF_SIZE;

    if (p_data) {
        tmemcpy(p_data, p_mic, len);
    }
}
#endif
