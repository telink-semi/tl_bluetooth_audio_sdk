...
static void tlkmdi_bt_voice_state_change_cb(uint16_t acl_handle, uint16_t sco_handle, uint8_t is_start, uint8_t codec)
{
    ...
    if (is_start) {
        ...
        if (tlkmdi_bt_tpt_isSlave()) {
            s_tlk_mdi_bt_voice_env.codec = btif_get_hfp_codec();
        } else {
        ...
        tlkmdi_audio_sendStartEvt(TLKAUD_TYPE_CC_BT_VOICE, s_tlk_mdi_bt_voice_env.acl_handle);
#endif
    } else {
        tlkapi_trace(TLKMDI_BT_VOICE_DBG_FLAG, TLKMDI_BT_VOICE_DBG_SIGN, "tlkmdi_bt_voice_disable");
        tlkmdi_audio_sendCloseEvt(TLKAUD_TYPE_CC_BT_VOICE, s_tlk_mdi_bt_voice_env.acl_handle);
