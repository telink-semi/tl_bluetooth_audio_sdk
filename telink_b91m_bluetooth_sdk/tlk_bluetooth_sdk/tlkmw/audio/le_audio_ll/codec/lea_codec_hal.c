...
{
#if LEA_CODEC_INPUT_MODE == LEA_CODEC_MODE_MONO
    uint16_t buffer_len_left = buffer_len / 2;
    uint8_t  left_data[buffer_len_left];
    if (tlkdrv_codec_readMicData(left_data, buffer_len_left, NULL) == false) {
        tlk_printf("readMicData fail");
        return false;
    }
    adc_mono_int *p_data = (adc_mono_int *)p_buffer;
    for (int i = 0; i < buffer_len_left / sizeof(adc_mono_int); i++) {
        p_data[2 * i + 1] = p_data[2 * i] = (adc_mono_int) * ((adc_mono_int *)left_data + i);
    }
#else
