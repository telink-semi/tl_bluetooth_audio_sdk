/********************************************************************************************************
 * @file    lea_codec_codec_no_asrc.c
 *
...
static void lea_codec_input_init_asrc_module(void)
{
    ...
    //        tlk_printf("codec input asrc buffer size:%d, ptr:%p", asrc_buffer_size, s_codec_input_asrc_buffer);
...
void lea_codec_input_stream_init(void)
{
    ...
    tlkdrv_open_codec(TLKDRV_CODEC_SUBDEV_MIC, TLKDRV_CODEC_CHANNEL_STEREO, TLKDRV_CODEC_BITDEPTH_16, s_codec_input_sampling_rate, 0);
}
...
void lea_codec_output_stream_init(void)
{
    ...
    tlkdrv_open_codec(TLKDRV_CODEC_SUBDEV_SPK, TLKDRV_CODEC_CHANNEL_STEREO, TLKDRV_CODEC_BITDEPTH_16, s_codec_output_sampling_rate, 0);
}
...
void lea_codec_output_set_audio_data(int16_t *left_data, int16_t *right_data, uint16_t sample_num)
{
    ...
    tlk_lea_codec_write_output_value((uint8_t *)pcm_stereo, sample_num * 4);
}
...
void lea_us_tone_continue(void)
{
    ...
    if (g_codec_rate == 8000) {
        sample_num = 80;
    } else if (g_codec_rate == 16000) {
        sample_num = 160;
    } else if (g_codec_rate == 24000) {
        sample_num = 240;
    } else if (g_codec_rate == 32000) {
        sample_num = 320;
    } else if (g_codec_rate == 48000) {
        sample_num = 480;
    }
    ...
    if (tone_is_playing()) {
        tone_get_sample((int16_t *)pData, sample_num * sizeof(tone_int), g_codec_rate);
    }
