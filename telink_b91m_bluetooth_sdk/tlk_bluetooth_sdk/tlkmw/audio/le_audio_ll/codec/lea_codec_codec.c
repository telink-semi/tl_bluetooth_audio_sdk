...
#if (!LEA_CODEC_NO_ASRC && TLK_MW_LE_AUDIO_LOW_LATENCY_ENABLE)
...
void lea_codec_input_stream_init(void)
{
    ...
    tlkdrv_open_codec(TLKDRV_CODEC_SUBDEV_MIC, TLKDRV_CODEC_CHANNEL_STEREO, TLKDRV_CODEC_BITDEPTH_20, 48000, 0);
#else
...
{
    tlkdrv_codec_sync_mic_samples(3 * 48);
}
 ...
 */
bool lea_codec_input_get_audio_data(adc_mono_int *left_data, adc_mono_int *right_data, uint16_t sample_num)
{
    adc_mono_int codec_pcm_stereo[480 * 2];
    ...
    adc_mono_int  pcm_stereo[sample_num * 2];
    adc_mono_int *p_pcm_stereo = codec_pcm_stereo;
    if (s_codec_input_asrc_type != TLKA_ASRC_FLAG_END) {
        tlka_asrc_16_bit_process_frame(s_codec_input_asrc_buffer, codec_pcm_stereo, sample_num_48k / sizeof(adc_int), pcm_stereo);
