...
#if (TLK_MW_LE_AUDIO_LOW_LATENCY_ENABLE && TLK_MW_LEA_US_ENABLE)
...
static uint8_t *s_alg_nn_ns_buffer = NULL;
#endif
#if TLKALG_AGC_ENABLE
static uint8_t *s_alg_agc_buffer = NULL;
#endif
#if TLKALG_EQ_ENABLE
static uint8_t *s_alg_eq_buff = NULL;
#endif
#if TLKALG_PPM_SPK_ENABLE
static uint8_t *s_alg_ppm_buff = NULL;
#endif
...
static void tlkmdi_bap_uc_event_callback(uint16_t conn_handle, enum ble_bap_us_event_id event, const void *event_msg)
{
    ...
    case BAP_US_EVENT_ID_OPEN_INPUT:
    {
        ...
        if (tlkmdi_lea_us_check_active_conn_handle(conn_handle)) {
            lea_open_input(ble_host_gap_iso_get_tx_time_stamp(p_input_param->iso_handle));
...
void tlkmdi_lea_us_alg_init(void)
{
#if TLKALG_NN_NS_ENABLE
    if (s_alg_nn_ns_buffer == NULL) {
        audio_alg_interface_t *p_audio_alg_nn = audio_alg_get_interface_by_type(ALG_NN_NS);
        uint16_t               nn_ns_mem_size = p_audio_alg_nn->audio_alg_get_size(ALG_CHANNEL_LEFT);
        s_alg_nn_ns_buffer                    = (uint8_t *)tlkalg_malloc_func(nn_ns_mem_size);
        if (s_alg_nn_ns_buffer == NULL) {
            tlkapi_printf(APP_LOG_EN, "nn ns buff alloc failed");
            return false;
        }
        p_audio_alg_nn->audio_alg_init(s_alg_nn_ns_buffer, ALG_CHANNEL_LEFT);

        tlkapi_printf(APP_LOG_EN, "nn_ns_mem_size: %d", nn_ns_mem_size);
    }
#endif

#if TLKALG_AGC_ENABLE
    if (s_alg_agc_buffer == NULL) {
        audio_alg_interface_t *p_audio_alg_nn = audio_alg_get_interface_by_type(ALG_AGC);
        uint16_t               agc_mem_size   = p_audio_alg_nn->audio_alg_get_size(ALG_CHANNEL_LEFT);
        s_alg_agc_buffer                      = (uint8_t *)tlkalg_malloc_func(agc_mem_size);
        if (s_alg_agc_buffer == NULL) {
            tlkapi_printf(APP_LOG_EN, "agc buff alloc failed");
            return false;
        }
        p_audio_alg_nn->audio_alg_init(s_alg_agc_buffer, ALG_CHANNEL_LEFT);

        tlkapi_printf(APP_LOG_EN, "agc_mem_size: %d", agc_mem_size);
    }
#endif
#if TLKALG_EQ_ENABLE
    if (s_alg_eq_buff == NULL) {
        ...
        p_audio_alg_if->audio_alg_init(s_alg_eq_buff, ALG_CHANNEL_LEFT);
    }
#endif
#if TLKALG_PPM_SPK_ENABLE
    if (s_alg_ppm_buff == NULL) {
        audio_alg_interface_t *p_audio_alg_ppm = audio_alg_get_interface_by_type(ALG_PPM_SPK);
        uint16_t               ppm_mem_size    = p_audio_alg_ppm->audio_alg_get_size(ALG_CHANNEL_LEFT);
        s_alg_ppm_buff                         = (uint8_t *)tlkalg_malloc_func(ppm_mem_size);
        if (s_alg_ppm_buff == NULL) {
            tlkapi_printf(APP_LOG_EN, "ppm buff alloc failed");
            return false;
        }
        p_audio_alg_ppm->audio_alg_init(s_alg_ppm_buff, ALG_CHANNEL_LEFT);

        tlkapi_printf(APP_LOG_EN, "ppm_mem_size: %d", ppm_mem_size);
    }
#endif
}

void tlkmdi_lea_us_alg_deinit(void)
{
#if TLKALG_NN_NS_ENABLE
    if (s_alg_nn_ns_buffer != NULL) {
        audio_alg_interface_t *p_audio_alg_nn = audio_alg_get_interface_by_type(ALG_NN_NS);
        tlkalg_free_func(s_alg_nn_ns_buffer);
        p_audio_alg_nn->audio_alg_deinit();
        s_alg_nn_ns_buffer = NULL;
    }
#endif
#if TLKALG_AGC_ENABLE
    if (s_alg_agc_buffer != NULL) {
        audio_alg_interface_t *p_audio_alg_nn = audio_alg_get_interface_by_type(ALG_AGC);
        tlkalg_free_func(s_alg_agc_buffer);
        p_audio_alg_nn->audio_alg_deinit();
        s_alg_agc_buffer = NULL;
    }
#endif
#if TLKALG_EQ_ENABLE
    if (s_alg_eq_buff != NULL) {
        audio_alg_interface_t *p_audio_alg_nn = audio_alg_get_interface_by_type(ALG_EQ);
...
#if TLKALG_PPM_SPK_ENABLE
    if (s_alg_ppm_buff != NULL) {
        audio_alg_interface_t *p_audio_alg_nn = audio_alg_get_interface_by_type(ALG_PPM_SPK);
        tlkalg_free_func(s_alg_ppm_buff);
        p_audio_alg_nn->audio_alg_deinit();
        s_alg_ppm_buff = NULL;
    }
#endif
...
static bool tlkmdi_lea_us_switch(uint16_t handle, uint8_t status)
{
    if (status == TLK_STATE_OPENED) {
        ...
        if (p_lea_us_audio_state->input_audio_index > 0) {
            ...
            lea_open_input(ble_host_gap_iso_get_tx_time_stamp(input_audio_init->iso_handle));
        ...
        if (codec_config.is_input_stream_init) {
            tlkmdi_lea_us_alg_init();
#if TLKALG_PPM_SPK_ENABLE
            tlkalg_lea_ppm_init(48000);
#endif
        }
#endif
