...
#include "stack/bt/hci/h0tl.h"
#include "stack/bt/common/co_error.h"
#include "stack/bt/common/rwip_config.h"
#include "stack/bt/common/co_hci.h"
#include "stack/bt/common/co_utils.h"
...
#if MCU_DUAL_CORE_ENABLE
static TlkBtHciEventCallback   sTlkBtHciEventCB   = 0;
static TlkBtHciAclDataCallback sTlkBtHciAclDataCB = 0;
static TlkBtHciScoDataCallback sTlkBtHciScoDataCB = 0;
...
void tlkbt_hci_recvC2HData(uint8_t *pData, uint32_t dataLen)
{
    ...
    if (type == HCI_ACL_MSG_TYPE) {
        ...
        if (sTlkBtHciAclDataCB != NULL) {
            sTlkBtHciAclDataCB(hldFlgs, pData + 5, length);
        }

    } else if (type == HCI_EVT_MSG_TYPE) {
        uint8_t evtCode = pData[1];
        uint8_t length  = pData[2];
        if (dataLen < (HCI_EVT_HDR_LEN + 1)) {
            return;
        }

        if (sTlkBtHciEventCB != NULL) {
            sTlkBtHciEventCB(evtCode, pData + 3, length);
        }
    } else if (type == HCI_SYNC_MSG_TYPE) {
        uint16_t hldFlgs = pData[1] | (pData[2] << 8);
        uint16_t length  = pData[3];

        if (sTlkBtHciScoDataCB != NULL) {
            sTlkBtHciScoDataCB(hldFlgs, pData + 4, length);
        }
    }
}
#endif
...
{
#if MCU_DUAL_CORE_ENABLE
    sTlkBtHciAclDataCB = cb;
#else
    rwbt_hci_regAclDataCB(cb);
#endif
}
...
{
#if MCU_DUAL_CORE_ENABLE
    sTlkBtHciScoDataCB = cb;
#else
    rwbt_hci_regScoDataCB(cb);
#endif
}
...
#endif
#else
    ret = h0tl_h2c_write_command(opcode, pData, dataLen);
#endif
    //send error
    if (ret != 0) {
        tlkapi_printf(DUMP_HCI_MSG, "!!tlkbt_hci_sendH2cCmd Fail ret = [%d] opcode = [0x%x]", ret, opcode);
        ret = -TLK_EQUOTA;
    }
    return ret;
}
...
int tlkbt_hci_sendH2cAclData(uint16_t aclHandle, uint8_t *pUsrExt, uint16_t extLen, uint8_t *pHead, uint16_t headLen, uint8_t *pData, uint16_t dataLen)
{
    ...
    if (bth_handle_getUsedAcl(aclHandle) == NULL) {
        tlkapi_printf(DUMP_HCI_MSG, "tlkbt_hci_sendH2cAclData: aclHandle error");
    ...
    if (extLen + headLen > TLKBT_HCI_H2C_ACL_SIZE || extLen + headLen + dataLen == 0 || aclHandle == 0) {
        tlkapi_printf(DUMP_HCI_MSG, "tlkbt_hci_sendH2cAclData: param error");
        return -TLK_EPARAM;
    }
    ...
    if ((pktCnt > avalPktNumb) || (avalPktNumb < TLKBT_HCI_H2C_ACL_RESV_NUMB)) {
        tlkapi_printf(DUMP_HCI_MSG, "tlkbt_hci_sendH2cAclData: not space");
        return -TLK_ENOSPACE;
    }
...
#if MCU_DUAL_CORE_ENABLE
    buffLen = 0;
    SETF(hdl_flags, HCI_ACL_HDR_HDL, aclHandle);
    SETF(hdl_flags, HCI_ACL_HDR_DATA_FLAGS, TLKBT_HCI_LLID_START);
    ...
    if (h0tl_h2c_write_acl_data(hdl_flags, buffLen, pBuffer) == 0) {
        bth_handle_aclPktNumbUpdate(aclHandle, true, 1);
        //tlkapi_printf(DUMP_HCI_MSG, "bth[%d] btc[%d]", tlkbt_hci_aclGetAvalSize(), bt_util_get_acl_tx_free_count());
    }
#endif
    ...
    while (tmpLen < dataLen) {
        ...
        buffLen = 0;
        SETF(hdl_flags, HCI_ACL_HDR_HDL, aclHandle);
        SETF(hdl_flags, HCI_ACL_HDR_DATA_FLAGS, TLKBT_HCI_LLID_CONTINUE);
...
uint32_t tlkbt_hci_h2cDatFifoUsed(void)
{
    ...
    return 0;
#else
    return h0tl_h2c_fifo_used();
#endif
}

uint32_t tlkbt_hci_h2cFifoSize(void)
{
    if (H0TL_H2C_FIFO_SIZE <= 4) {
        return 0;
    } else {
        return (H0TL_H2C_FIFO_SIZE - 4);
    }
}
