...
namespace lc3
{
    ...
    template <typename T> class Base
    {
      protected:
        Base(int dt_us, int sr_hz, int sr_pcm_hz, size_t nchannels):
            dt_us_(dt_us), sr_hz_(sr_hz), sr_pcm_hz_(sr_pcm_hz == 0 ? sr_hz : sr_pcm_hz), nchannels_(nchannels)
        {
    ...
    {
        template <typename T> int EncodeImpl(PcmFormat fmt, const T *pcm, int frame_size, uint8_t *out)
        {
            if (states.size() != nchannels_) {
                return -1;
            }
            ...
            for (size_t ich = 0; ich < nchannels_; ich++) {
                ret |= lc3_encode(states[ich].get(), cfmt, pcm + ich, nchannels_, frame_size, out + ich * frame_size);
            }
      ...
      public:
        ...
        {
            for (size_t ich = 0; ich < nchannels_; ich++) {
                auto s = state_ptr((lc3_encoder_t)malloc(lc3_encoder_size(dt_us_, sr_pcm_hz_)), free);
        ...
        //
        // The value returned is 0 on success, -1 otherwise.

        int Encode(const int16_t *pcm, int frame_size, uint8_t *out) { return EncodeImpl(PcmFormat::kS16, pcm, frame_size, out); }

        int Encode(const int32_t *pcm, int frame_size, uint8_t *out) { return EncodeImpl(PcmFormat::kS24, pcm, frame_size, out); }

        int Encode(const float *pcm, int frame_size, uint8_t *out) { return EncodeImpl(PcmFormat::kF32, pcm, frame_size, out); }
        ...
        int Encode(PcmFormat fmt, const void *pcm, int frame_size, uint8_t *out)
        {
            ...
            case PcmFormat::kS16:
                ...
                return EncodeImpl(fmt, reinterpret_cast<const int16_t *>(pcm), frame_size, out);

            case PcmFormat::kS24:
                assert(pcm_ptr % alignof(int32_t) == 0);
                return EncodeImpl(fmt, reinterpret_cast<const int32_t *>(pcm), frame_size, out);

            case PcmFormat::kS24In3Le:
                return EncodeImpl(fmt, reinterpret_cast<const int8_t(*)[3]>(pcm), frame_size, out);

            case PcmFormat::kF32:
                assert(pcm_ptr % alignof(float) == 0);
                return EncodeImpl(fmt, reinterpret_cast<const float *>(pcm), frame_size, out);
            }
    ...
    {
        template <typename T> int DecodeImpl(const uint8_t *in, int frame_size, PcmFormat fmt, T *pcm)
        {
            if (states.size() != nchannels_) {
                return -1;
            }
            ...
            for (size_t ich = 0; ich < nchannels_; ich++) {
                ret |= lc3_decode(states[ich].get(), in + ich * frame_size, frame_size, cfmt, pcm + ich, nchannels_);
            }
      ...
      public:
            ...
            Base(dt_us, sr_hz, sr_pcm_hz, nchannels)
        {
            for (size_t i = 0; i < nchannels_; i++) {
                auto s = state_ptr((lc3_decoder_t)malloc(lc3_decoder_size(dt_us_, sr_pcm_hz_)), free);
        ...
        //
        // The value returned is 0 on success, 1 when PLC has been performed,
        // and -1 otherwise.

        int Decode(const uint8_t *in, int frame_size, int16_t *pcm) { return DecodeImpl(in, frame_size, PcmFormat::kS16, pcm); }

        int Decode(const uint8_t *in, int frame_size, int32_t *pcm) { return DecodeImpl(in, frame_size, PcmFormat::kS24In3Le, pcm); }

        int Decode(const uint8_t *in, int frame_size, float *pcm) { return DecodeImpl(in, frame_size, PcmFormat::kF32, pcm); }
        ...
        int Decode(const uint8_t *in, int frame_size, PcmFormat fmt, void *pcm)
        {
            ...
            case PcmFormat::kS16:
                assert(pcm_ptr % alignof(int16_t) == 0);
                return DecodeImpl(in, frame_size, fmt, reinterpret_cast<int16_t *>(pcm));

            case PcmFormat::kS24:
                assert(pcm_ptr % alignof(int32_t) == 0);
                return DecodeImpl(in, frame_size, fmt, reinterpret_cast<int32_t *>(pcm));
