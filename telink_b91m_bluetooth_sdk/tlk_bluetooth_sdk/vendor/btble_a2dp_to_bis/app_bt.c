...
static int app_btmgr_aclDisconnCB(uint8_t *pData, uint16_t dataLen)
{
    ...
    uint32_t taskID = event->aclHandle + ((uint32_t)TLKAUD_TYPE_A2DP_TO_BIS << 16);
    tlkapp_audioScheduler_deleteTask(taskID);
    taskID = event->aclHandle + ((uint32_t)TLKAUD_TYPE_CC_BT_MUSIC << 16);
    tlkapp_audioScheduler_deleteTask(taskID);
 ...
 * @return      TLK_ENONE is success, others is failure
 */
static int app_btmgr_aclConnectRoleCB(uint8_t *pData, uint16_t dataLen)
{
    ...
    if (isRequest) {
        return bth_acl_setInitRole(event->pBtAddr, BTH_ROLE_MASTER);
    } else {
        return bth_acl_connect(event->pBtAddr, event->devClass, BTH_ROLE_MASTER, event->timeout);
    }
}
#endif
...
#if (!TLK_CFG_BT_ROLE_DEFAULT)
TLKMW_BT_EVT_REGISTER(TLKMW_BT_SET_LOCAL_ROLE, app_btmgr_aclConnectRoleCB);
#endif
