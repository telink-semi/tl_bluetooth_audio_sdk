...
struct app_bis_sink_info
{
    ...
    app_ble_bis_sink_operate_callback operate_callback;
...
static const struct ble_vcss_register_param s_vcss_param = {
    .step   = 15,
    .volume = 150,
    .mute   = false,
};
...
/**
 * @brief       Handle ACL disconnection for BIS sink
 * @param[in]   conn - Connection information
...
{
    (void)conn;
    tlkapi_printf(APP_LOG_EN, "[APP][BIS] ACL disconnected, reason=0x%x", reason);

    s_app_bis_sink_info.is_connected = false;

    if (s_app_bis_sink_info.operate_callback != NULL) {
        ble_host_gap_extend_adv_delete_set(s_app_bis_sink_info.ext_adv_handle);
        app_ble_bis_sink_operate_callback callback = s_app_bis_sink_info.operate_callback;
...
/**
 * @brief       Handle BIS sink events
 * @param[in]   event - Event ID
 * @param[in]   event_msg - Event message data
 * @return      none
 */
...
void app_ble_bis_sink_init(void)
{
    ...
    s_app_bis_sink_info.device_name_len = snprintf(s_app_bis_sink_info.device_name, BIS_SINK_DEVICE_NAME_MAX_SIZE, BIS_SINK_DEVICE_NAME "-%02X%02X%02X%02X%02X%02X", bd_addr[5],
                                                   bd_addr[4], bd_addr[3], bd_addr[2], bd_addr[1], bd_addr[0]);
...
/**
 * @brief       Start advertisement for BLE BIS sink
 * @param       none
 * @return      none
 */
static void app_ble_bis_sink_start_adv(void)
{
    ...
    tlkapi_printf(APP_LOG_EN, "[APP][INI] Sink set adv enable, status=0x%x", status);
}
