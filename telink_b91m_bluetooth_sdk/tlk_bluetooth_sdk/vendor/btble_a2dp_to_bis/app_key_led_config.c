...
#if (D25F_TEST_MODE == D25F_BTBLE_A2DP_TO_BIS)
...
static const uint16_t sApp_key_default_config[TLKDRV_KEY_MAX_NUMB][TLKDRV_KEY_EVTID_MAX] = {
    ...
    [3] =
        {
            [TLKDRV_KEY_EVTID_CLICK] = KEY_EVT_VENDOR_CONFIG_2,
        },
#endif
};
 ...
 * @param       none
 * @return      none
 * @note        Sets up default key mappings if no configuration exists or if
 *              current configuration differs from default
 */
 ...
 * @param[in]   key_mode - Key mode to configure
 * @return      none
 * @note        Updates key configuration in persistent storage based on user input
 */
 ...
 * @param       none
 * @return      none
 * @note        Initializes default key configuration when system key initialization completes
 */
...
#endif // #if (D25F_TEST_MODE == D25F_BTBLE_A2DP_TO_BIS)