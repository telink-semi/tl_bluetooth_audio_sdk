...
/**
 * @brief       Append profiles to the ACL connection
 * @param[in]   aclHandle - ACL connection handle
 * @return      none
 * @note        This function appends various profiles (RFC, HFP, PBAP, A2DP, AVRCP, etc.) 
 *              to the connection based on device type and capabilities
 */
 ...
 * @param[in]   pBtAddr - Pointer to Bluetooth address
 ...
 * @param[in]   pBtAddr - Pointer to Bluetooth address
 * @return      none
 * @note        This function handles ACL disconnection events, including audio task cleanup,
 *              IAP handle management, and initiating reconnection if needed
 */
 ...
 * @return      none
 * @note        This function handles profile connection events, setting up audio tasks
 *              and connecting AVRCP profile for A2DP connections
 */
...
#endif // #if (TLKBTP_CFG_PBAP_ENABLE)
#if (TLKBTP_CFG_IAP_ENABLE)
    else if (ptype == BTP_PTYPE_IAP) {
        tlkmdi_tinySql_setPairingDeviceRfcChid(pBtAddr, 0x01, TLKMDI_BT_RFC_GATT_SUPPORT);
    }
#endif
    if (isFirstProf) {
 ...
 * @param[in]   pBtAddr - Pointer to Bluetooth address
 * @return      none
 * @note        This function handles profile disconnection events, cleaning up audio tasks
 *              and managing IAP connections
 */
...
static void app_btmgr_appendProfile(uint16_t aclHandle)
{
    ...
    if (tlkmdi_btacl_isHaveRfc(aclHandle)) {
        ...
        if (dtype != BTH_REMOTE_DTYPE_HEADSET && tlkmdi_btacl_isFindIap(aclHandle)) {
            tlkmdi_btacl_appendProf(aclHandle, BTP_PTYPE_IAP, BTP_USRID_CLIENT, delayMs + 200);
        }
        delayMs += 800;
    }
    ...
    uint8_t gatt_support = 0;
    tlkmdi_tinySql_getPairingDeviceRfcChid(pItem->btaddr, &gatt_support, TLKMDI_BT_RFC_GATT_SUPPORT);
    if (pItem->active && gatt_support) {
        tlkmdi_btacl_appendProf(aclHandle, BTP_PTYPE_ATT, BTP_USRID_NONE, delayMs + 500);
    }
}
