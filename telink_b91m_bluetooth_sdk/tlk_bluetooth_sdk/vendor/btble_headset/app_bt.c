...
static int app_btmgr_ProfConnCB(uint8_t *pData, uint16_t dataLen)
{
    ...
    else if (event->ptype == BTP_PTYPE_PBAP) {
...
#endif // #if (TLKBTP_CFG_PBAP_ENABLE)
#if (TLKBTP_CFG_IAP_ENABLE)
    else if (event->ptype == BTP_PTYPE_IAP) {
        tlkmdi_tinySql_setPairingDeviceRfcChid(event->pBtAddr, 0x01, TLKMDI_BT_RFC_GATT_SUPPORT);
    }
#endif
