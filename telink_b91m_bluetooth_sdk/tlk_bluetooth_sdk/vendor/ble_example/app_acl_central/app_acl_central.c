...
int INIT(APP_BLE_ACL_CENTRAL)(void)
{
    ...
    tlk_printf("[APP] BLE ACL Central initialized");
    return 0;
}

void START(APP_BLE_ACL_CENTRAL)(void) {}
...
static void app_acl_central_create_new_connection(const struct ble_host_gap_evt_scan_adv_report_undirected *p_adv_report)
{
    ...
    if (ret == BLE_HOST_ERR_SUCC) {
        ble_host_gap_acl_central_create_connection(
