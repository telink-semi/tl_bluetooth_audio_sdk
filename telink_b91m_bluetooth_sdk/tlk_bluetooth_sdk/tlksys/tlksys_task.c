...
static TlkOsMsgQHandle_t     sTlkTaskMsgQHandles[TLKSYS_TASKID_MAXNUM]   = {0};
static TlkOsEventTabHandle_t sTlkTaskEvtTabHandles[TLKSYS_TASKID_MAXNUM] = {0};
...
static void tlksys_task_handleMsg(const tlksys_task_t *pTask)
{
    while (1) {
        uint8_t buffer[128];
        int     res = tlkos_msgq_wait(sTlkTaskMsgQHandles[pTask->taskID], buffer, NULL, 128, 0);
...
static bool tlksys_task_isAllIdle(void)
{
    for (size_t index = 0; index < TLKSYS_TASKID_MAXNUM; index++) {
        ...
        tlkos_event_get(sTlkTaskEvtTabHandles[index], &evt);
        if (evt) {
            return false;
        }
    }
...
int tlksys_task_create(uint16_t taskID, const tlksys_task_cfg_t *pCfgs)
{
    ...
    TlkosTaskExtCfg_t extCfg = {
        .pStaticBuffer    = pTask->pCfgs->pTaskStaticBuffer,
        .staticBufferSize = pTask->pCfgs->stackSize,
    };
...
{
    tlkos_event_regDealCB(sTlkTaskEvtTabHandles[taskID], index, cb);
    tlkos_event_set(sTlkTaskEvtTabHandles[taskID], index);
}
...
{
    tlkos_event_set(sTlkTaskEvtTabHandles[taskID], id);
}
...
{
    tlkos_event_setFromIsr(sTlkTaskEvtTabHandles[taskID], id);
}
...
{
    tlkos_event_wait(sTlkTaskEvtTabHandles[taskID], blockTimeMs);
}
