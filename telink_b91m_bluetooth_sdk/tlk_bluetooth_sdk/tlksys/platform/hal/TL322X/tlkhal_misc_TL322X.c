...
#define TLKHAL_MAC_COMPANY_ID 0xBC9D37
...
void tlkhal_get_bluetooth_mac(uint8_t *mac)
{
    ...
    // Step 1: Read MAC from eFuse. Use it if programmed.
    if (efuse_get_mac_address(mac_flash, 6)) {
        memcpy(mac, mac_flash, sizeof(mac_flash));
        return;
    }

    // Step 2: Generate random MAC and save to flash.
    // Note: Random MAC is for internal debugging only. In mass production, eFuse must be programmed with a valid MAC.
